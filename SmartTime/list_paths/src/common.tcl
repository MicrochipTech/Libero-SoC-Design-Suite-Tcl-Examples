...
set list_paths "./list_paths.txt"
set map_var(0) "Path 1"
set map_var(1) "source_pin"
set map_var(2) "sink_pin"
...
proc grepPattern { ex num} \
{
	...
	while {[eof $f_id_1]==0} \
	{
		...
		if {[regexp $ex "$line_tokens"]==1} \
		{
			...
			puts "\nINFO: $map_var($num) is found."
			set found 1	
		}
	}
	
	if { $found == 0 } {
	puts "\nINFO: $map_var($num) is not found."
	incr err
	set found 0	
	}
}
