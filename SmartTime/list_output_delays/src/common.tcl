...
set list_output_delays_report "./list_output_delays.txt"
set map_var(0) "CLKIN"
...
proc grepPattern { ex num} \
{
	...
	global list_output_delays_report
	global map_var
	global found
	global err
	set f_id_1 [open $list_output_delays_report]
	...
	while {[eof $f_id_1]==0} \
	{
		...
		if {[regexp $ex "$line_tokens"]==1} \
		{
			...
			puts "\nINFO: Output Delay set on $map_var($num) is reported"
			set found 1	
		}
	}
	
	if { $found == 0 } {
	puts "\nERROR: Output Delay set on $map_var($num) is not reported"
	incr err
	set found 0	
	}
}
