...
set list_output_delays "./list_output_delays.txt"
set map_var(0) "output_delay"
...
proc grepPattern { ex num} \
{
	...
	global list_output_delays
	global map_var
	global found
	global err
	set f_id_1 [open $list_output_delays]
	...
	while {[eof $f_id_1]==0} \
	{
		...
		if {[regexp $ex "$line_tokens"]==1} \
		{
			...
			puts "\nINFO: $map_var($num) is found and indicates that remove_output_delay command is not working"
			set found 1	
		}
	}
	
	if { $found == 0 } {
	puts "\nINFO: $map_var($num) is not found and indicates that remove_output_delay command is not working"
	incr err
	set found 0	
	}
}
