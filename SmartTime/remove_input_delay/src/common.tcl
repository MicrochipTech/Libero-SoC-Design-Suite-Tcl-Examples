...
set list_input_delay_report "./list_input_delays.txt"
set map_var(0) "din_a"
...
proc grepPattern { ex num} \
{
	...
	global list_input_delay_report
	global map_var
	global found
	global err
	set f_id_1 [open $list_input_delay_report]
	...
	while {[eof $f_id_1]==0} \
	{
		...
		if {[regexp $ex "$line_tokens"]==1} \
		{
			...
			puts "\nINFO: $map_var($num) is found and indicates that input delay constraints from $map_var($num) has not been removed"
			set found 1	
			incr err
		}
	}
	...
	if { $found == 0 } {
	puts "\nINFO: $map_var($num) is not found and indicates that input delay constraints from $map_var($num) has been removed"
	set found 0	
	}
}
##########################################################
