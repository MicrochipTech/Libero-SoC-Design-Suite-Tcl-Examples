...
set current_scenario "./current_scenario.txt"
...
proc grepPattern { ex num} \
{
	...
	global found
	global err
	set f_id_1 [open $current_scenario]
	...
	while {[eof $f_id_1]==0} \
	{
		...
		if {[regexp $ex "$line_tokens"]==1} \
		{
			...
			puts "\nINFO: $map_var($num) is Set current timing scenario."
			set found 1	
		}
	}
	
	if { $found == 0 } {
	puts "\nINFO: $map_var($num) is not Set current timing scenario."
	incr err
	set found 0	
	}
}
