...
set list_clock_latencies_report "./list_input_delays.txt"
set map_var(0) "CLKIN"
...
proc grepPattern { ex num} \
{
	...
	while {[eof $f_id_1]==0} \
	{
		...
		if {[regexp $ex "$line_tokens"]==1} \
		{
			...
			puts "\nINFO: Input Delay set on $map_var($num) is reported"
			set found 1	
		}
	}
	
	if { $found == 0 } {
	puts "\nERROR: Input Delay set on $map_var($num) is not reported"
	incr err
	set found 0	
	}
}
