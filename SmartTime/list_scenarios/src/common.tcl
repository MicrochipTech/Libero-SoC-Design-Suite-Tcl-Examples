...
set list_scenarios "./list_scenarios.txt"
...
proc grepPattern { ex num} \
{
	...
	global list_scenarios
	global map_var
	global found
	global err
	set f_id_1 [open $list_scenarios]
	...
	while {[eof $f_id_1]==0} \
	{
		...
		if {[regexp $ex "$line_tokens"]==1} \
		{
			...
			puts "\nINFO: $map_var($num) is found and indicates that the specified timing scenario is found."
			set found 1	
		}
	}
	
	if { $found == 0 } {
	puts "\nINFO: $map_var($num) is not found and indicates that the specified timing scenario is not found."
	incr err
	set found 0	
	}
}
