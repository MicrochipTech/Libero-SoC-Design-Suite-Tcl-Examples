...
set list_clock_latencies_report "./list_clock_latencies.txt"
set map_var(0) "CLKIN"
...
proc grepPattern { ex num} \
{
	...
	global list_clock_latencies_report
	global map_var
	global found
	global err
	set f_id_1 [open $list_clock_latencies_report]
	...
	while {[eof $f_id_1]==0} \
	{
		...
		if {[regexp $ex "$line_tokens"]==1} \
		{
			...
			puts "\nINFO: $map_var($num) is found and indicates that clock source latency from the CLKIN has not been removed"
			set found 1	
		}
	}
	
	if { $found == 0 } {
	puts "\nINFO: $map_var($num) is not found and indicates that clock source latency from the CLKIN has been removed"
	incr err
	set found 0	
	}
}
