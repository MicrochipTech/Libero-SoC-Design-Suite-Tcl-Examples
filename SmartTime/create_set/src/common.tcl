...
proc grepPattern { ex num} \
{
	...
	while {[eof $f_id_1]==0} \
	{
		...
		if {[regexp $ex "$line_tokens"]==1} \
		{
			...
			puts "\nINFO: $map_var($num) is reported and indicates that the set has been created"
			set found 1
		}
	}
	
	if { $found == 0 } {
	puts "\nERROR: $map_var($num) is not reported and indicates that the set has not been created"
	incr err
	set found 0
	}
