...
proc grepPattern { ex num} \
{
	...
	while {[eof $f_id_1]==0} \
	{
		...
		if {[regexp $ex "$line_tokens"]==1} \
		{
			...
			puts "\nINFO: $map_var($num) is found and indicates that clock group has not been removed"
			set found 1	
		}
	}
	
	if { $found == 0 } {
	puts "\nINFO: $map_var($num) is not found and indicates that clock group has been removed"
	incr err
	set found 0	
	}
}
