...
set list_disable_timings_report "./list_disable_timings.txt"
...
proc grepPattern { ex num} \
{
	...
	global list_disable_timings_report
	global map_var
	global found
	global err
	set f_id_1 [open $list_disable_timings_report]
	...
	while {[eof $f_id_1]==0} \
	{
		...
		if {[regexp $ex "$line_tokens"]==1} \
		{
			...
			puts "\nINFO: $map_var($num) is found and indicates that disable timing constraint has not been removed"
			set found 1	
		}
	}
	
	if { $found == 0 } {
	puts "\nINFO: $map_var($num) is not found and indicates that disable timing constraint has been removed"
	incr err
	set found 0	
	}
}
