...
set remove_scenario "./remove_scenario.txt"
set map_var(0) "scenario_A"
...
proc grepPattern { ex num} \
{
	...
	global remove_scenario
	global map_var
	global found
	global err
	set f_id_1 [open $remove_scenario]
	...
	while {[eof $f_id_1]==0} \
	{
		...
		if {[regexp $ex "$line_tokens"]==1} \
		{
			...
			puts "\nINFO: $map_var($num) is found and indicates that the specified timing scenario is not removed."
			set found 1	
		}
	}
	
	if { $found == 0 } {
	puts "\nINFO: $map_var($num) is not found and indicates that the specified timing scenario is removed."
	incr err
	set found 0	
	}
}
