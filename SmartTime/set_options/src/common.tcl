...
set get_current_scenario "./get_current_scenario.txt"
set map_var(0) "current scenario"
...
proc grepPattern { ex num} \
{
	...
	global get_current_scenario
	global map_var
	global found
	global err
	set f_id_1 [open $get_current_scenario]
	...
	while {[eof $f_id_1]==0} \
	{
		...
		if {[regexp $ex "$line_tokens"]==1} \
		{
			...
			puts "\nINFO: $map_var($num) is $ex implies set options command works."
			set found 1	
		}
	}
	
	if { $found == 0 } {
	puts "\nINFO: $map_var($num) is not $ex implies set options command does not work."
	incr err
	set found 0	
	}
}
