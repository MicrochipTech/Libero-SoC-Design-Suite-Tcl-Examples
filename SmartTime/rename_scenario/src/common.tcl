...
set rename_scenario "./rename_scenario.txt"
set map_var(0) "scenario_A_1"
...
proc grepPattern { ex num} \
{
	...
	global rename_scenario
	global map_var
	global found
	global err
	set f_id_1 [open $rename_scenario]
	...
	while {[eof $f_id_1]==0} \
	{
		...
		if {[regexp $ex "$line_tokens"]==1} \
		{
			...
			puts "\nINFO: $map_var($num) is found and indicates that the specified timing scenario is renamed."
			set found 1	
		}
	}
	
	if { $found == 0 } {
	puts "\nINFO: $map_var($num) is not found and indicates that the specified timing scenario is not renamed."
	incr err
	set found 0	
	}
}
