...
proc grepPattern { ex num} \
{
	...
	while {[eof $f_id_1]==0} \
	{
		...
		if {[regexp $ex "$line_tokens"]==1} \
		{
			...
			puts "\nINFO:_TC $map_var($num) value is Matched with SDC Value in max analysis"
			set found 1	
		}
	}
	
	if { $found == 0 } {
	puts "\nERROR:_TC $map_var($num) value is not Matched with SDC Value in max analysis"
	incr err
	set found 0	
	}
	...
	while {[eof $f_id_2]==0} \
	{
		...
		if {[regexp $ex "$line_tokens"]==1} \
		{
			puts "\nINFO:_TC $map_var($num) value is Matched with SDC Value in min analysis"
			set found 1	
		}
	}
	
	if { $found == 0 } {
	puts "\nERROR:_TC $map_var($num) value is not Matched with SDC Value in min analysis"
	incr err
	set found 0	
	}
}
