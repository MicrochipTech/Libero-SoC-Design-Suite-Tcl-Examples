...
set list_clocks_report "./list_clocks.txt"
set map_var(0) "CLKIN1"
...
proc grepPattern { ex num} \
{
	...
	global list_clocks_report
	global map_var
	global found
	global err
	set f_id_1 [open $list_clocks_report]
	...
	while {[eof $f_id_1]==0} \
	{
		...
		if {[regexp $ex "$line_tokens"]==1} \
		{
			...
			puts "\nINFO: $map_var($num) is found and indicates that the specified clock constraint is not removed from the current timing scenario."
			set found 1	
		}
	}
	
	if { $found == 0 } {
	puts "\nINFO: $map_var($num) is not found and indicates that the specified clock constraint is removed from the current timing scenario."
	incr err
	set found 0	
	}
}
